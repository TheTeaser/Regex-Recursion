...
function ioEmail(email){
    // Add your logic.
    let regRul= /^[a-zA-z0-9]+@[a-zA-z0-9.-]+\.(io)$/;

    return regRul.test(email);
}
...
function imagesSearcher(text){
    let arr = [];
    // Add your logic. /([a-zA-Z0-9]+\.(jpg|jpeg|png))/g;
    let regRule= /([a-zA-Z0-9]+\.(jpg|jpeg|png))/g;
    arr= text.match(regRule);
    if (arr)
    {
    return arr
    }
